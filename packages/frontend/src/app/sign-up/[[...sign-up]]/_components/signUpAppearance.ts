/**
 * Clerk <SignUp /> appearance for the neon / aurora design system.
 *
 * Why this file mirrors signInAppearance one-to-one: the two Clerk
 * components share the same primitive elements, so the same map
 * produces a consistent form. The files stay separate so they can
 * diverge later (e.g. onboarding microcopy).
 *
 * Token source of truth: tailwind.config.js (`neon.*`) and globals.css
 * (gradient / glass / shadow utilities) — mirror any token move here,
 * since Clerk only accepts literal CSS values.
 */

import type { SignUp } from '@clerk/nextjs';

/**
 * The shape Clerk's <SignUp /> expects on its `appearance` prop. We
 * derive it from the component so this file stays in sync if Clerk
 * renames or narrows the type.
 */
type SignUpAppearance = NonNullable<
  Parameters<typeof SignUp>[0]['appearance']
>;

/**
 * Design-token mirror for the neon / aurora system. Same
 * values as signInAppearance.TOKENS — kept duplicated (not imported) so
 * the two files can diverge later if needed without a cross-file edit.
 *
 * If we ever change the brand colors, BOTH places (signInAppearance +
 * this file) must move together with tailwind.config.js.
 */
const TOKENS = {
  ...
  // Non-brand semantic colors — legible on near-black, not in the brand palette.
  danger: '#F87171',
  success: '#34D399',
} as const;

export const signUpAppearance: SignUpAppearance = {
  // -----------------------------------------------------------------
  ...
  options: {
    ...
    // First/last name on the sign-up factor-one step — these are useful
    // here for tailoring the per-question evaluation, unlike sign-in
    // where we don't want to surface them.
  ...
  // -----------------------------------------------------------------
  elements: {
    // The SignUp root container — already flush via `elevation`,
    ...
    // size — the SignUpShell wrapper is sized to fit the card width,
    ...
    // provider. Hidden so the badge doesn't get clipped by the
    // parent `overflow-hidden` card.
    ...
    // Footer action row ("Already have an account? Sign in" / etc.) —
